...
fn check_aligned(addr: usize, align: usize) -> bool {
    addr == (addr & !(align - 1))
}
...
unsafe impl GlobalAlloc for FreeListAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        while !curr.is_null() {
            ...
            if check_aligned(curr_addr, align) && (*curr).size >= size {
        ...
        let mut curr = self.bump_next.load(core::sync::atomic::Ordering::SeqCst);
        loop {
            let aligned = align_up(curr, align);
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        block_ptr.write(FreeBlock { size
