...
pub fn check_permission(pte: u64, read: bool, write: bool, execute: bool) -> bool {
    ...
    let read_checked = !read || (read && pte & PTE_R != 0);
    let write_checked = !write || (write && pte & PTE_W != 0);
    let execute_checked = !execute || (write && pte & PTE_X != 0);
    is_valid(pte) && read_checked && write_checked && execute_checked
}
