...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        ...
        unsafe {
            *((sp + 8) as *mut usize) = thread_finished as usize;
    ...
    pub fn run(&mut self) {
        ...
        loop {
            let finished = self.threads[1..].iter()
    ...
    fn schedule_next(&mut self) {
        ...
        for i in 1..=len {
            let next = (self.current + i) % len;
            if self.threads[next].state == ThreadState::Ready {
                // 保存当前线程状态（如果不是 Finished）
                ...
                // 设置 CURRENT_THREAD_ENTRY（如果是新线程）
                ...
                // 获取上下文指针
                let old_ctx = &mut self.threads[self.current].ctx as *mut TaskContext;
                let new_ctx = &self.threads[next].ctx as *const TaskContext;
